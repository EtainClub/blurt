...
{{
   public:
      {plugin_name}_api( const blurt::app::api_context& ctx );
...
class {plugin_name}_plugin : public blurt::app::plugin
{{
   public:
      {plugin_name}_plugin( blurt::app::application* app );
...
"{plugin_name}_api.cpp" :
"""
#include <blurt/app/api_context.hpp>
#include <blurt/app/application.hpp>
...
{{
   public:
      {plugin_name}_api_impl( blurt::app::application& _app );

      std::shared_ptr< {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin > get_plugin();

      blurt::app::application& app;
}};

{plugin_name}_api_impl::{plugin_name}_api_impl( blurt::app::application& _app ) : app( _app )
{{}}
...
{plugin_name}_api::{plugin_name}_api( const blurt::app::api_context& ctx )
{{
...
{{
   public:
      {plugin_name}_plugin_impl( blurt::app::application& app );
...
{plugin_name}_plugin_impl::{plugin_name}_plugin_impl( blurt::app::application& app )
  : _app(app) {{}}
...
{plugin_name}_plugin::{plugin_name}_plugin( blurt::app::application* app )
   : plugin(app)
{{
...
BLURT_DEFINE_PLUGIN( {plugin_name}, {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin )
""",
}
