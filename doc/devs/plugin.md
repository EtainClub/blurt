...
All plugins in the `libraries/plugins` directory are iterated over by `CMakeLists.txt`. The manifest directory iterates through all plugins, adding them to the `blurt_plugins` build target. Any other build target wanting to access all plugins
...
- By default, blurtd runs the `chain`, `p2p`, and `webserver` plugins.
