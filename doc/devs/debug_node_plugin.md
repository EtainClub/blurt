...
Anyone (even you!) can edit their account to contain 1 million BLURT.  It's really
...
yourself 1 million BLURT.  Just like it's really impossible for your bank to physically stop
...
the fake BLURT or fake dollars you "gave" yourself).  So you can believe whatever you want
about your balance and rewrite the rules of your own bookkeeping system to show you whatever balance you
want to be shown, but as soon as you try to actually spend BLURT (or dollars) that you don't actually have,
...
(see `libraries/plugins/debug_node/include/blurt/plugins/debug_node/debug_node_api.hpp`
...
Okay, let's run `blurtd`.  It should start immediately with no blocks.  We can ask it to read blocks from the directory we saved earlier:
...
The important information in the above is:
    ...
    ... "signing_key":"BLT6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV" ...
...
If we want to take control of an account we can do so by editing its key with `debug_update_object` command like this:
    ...
    curl --data '{"jsonrpc": "2.0", "method": "call", "params": [2,"debug_update_object",[{"_action":"update","id":"2.2.28","active":{"weight_threshold":1,"key_auths":[["BLT6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",1]]}}]], "id": 9}]' http://127.0.0.1:8090/rpc
...
Now that we've reset its key, we can take control of it in the wallet:
    ...
    list_my_accounts
    transfer steemit dantheman "1.234 BLURT" "make -j100 money" true
    list_my_accounts
