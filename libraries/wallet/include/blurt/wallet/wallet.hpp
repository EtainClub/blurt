...
class wallet_api
{
   public:
       ...
       *                       operations defined in `blurt/chain/operations.hpp`
       ...
       * @param daily_pay  - the amount of BLURT_SYMBOL that is being requested to be paid out daily,
...
FC_REFLECT( blurt::wallet::wallet_data,
            (cipher_keys)
            (ws_server)
            (ws_user)
            (ws_password)
...
FC_REFLECT( blurt::wallet::brain_key_info, (brain_priv_key)(wif_priv_key) (pub_key))

FC_REFLECT( blurt::wallet::plain_keys, (checksum)(keys) )

FC_REFLECT_ENUM( blurt::wallet::authority_type, (owner)(active)(posting) )

FC_API( blurt::wallet::wallet_api,
        /// wallet api
        (help)(gethelp)
