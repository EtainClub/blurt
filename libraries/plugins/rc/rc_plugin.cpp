...
#include <blurt/plugins/rc/rc_curve.hpp>
#include <blurt/plugins/rc/rc_export_objects.hpp>
...
#include <blurt/jsonball/jsonball.hpp>
...
// 2 / ( 24 * 5 ) = 0.01666...
#define BLURT_RC_MAX_NEGATIVE_PERCENT 166

namespace blurt { namespace plugins { namespace rc {

using blurt::plugins::block_data_export::block_data_export_plugin;

namespace detail {

using chain::plugin_exception;
using blurt::chain::util::manabar_params;
...
void create_rc_account( database& db, uint32_t now, const account_object& account, asset max_rc_creation_adjustment )
{
   ...
   if( max_rc_creation_adjustment.symbol == BLURT_SYMBOL )
   {
   ...
   )
{
   ...
   if( account_name == account_name_type() )
   {
      if( db.is_producing() )
      {
         BLURT_ASSERT( false, plugin_exception,
   ...
   mbparams.regen_time = BLURT_RC_REGEN_TIME;
   ...
   db.modify( rc_account, [&]( rc_account_object& rca )
   {
      ...
      int64_t min_mana = -1 * ( BLURT_RC_MAX_NEGATIVE_PERCENT * mbparams.max_mana ) / BLURT_100_PERCENT;
...
{ try {
   ...
   int64_t rc_regen = (gpo.total_vesting_shares.amount.value / (BLURT_RC_REGEN_TIME / BLURT_BLOCK_INTERVAL));
   ...
   if( rc_regen > 0 )
   {
      for( size_t i=0; i<BLURT_NUM_RESOURCE_TYPES; i++ )
      {
   ...
   std::shared_ptr< exp_rc_data > export_data =
      blurt::plugins::block_data_export::find_export_data< exp_rc_data >( BLURT_RC_PLUGIN_NAME );
      ...
      [&]( rc_pool_object& pool_obj )
      {
         ...
         for( size_t i=0; i<BLURT_NUM_RESOURCE_TYPES; i++ )
         {
            ...
            if( debug_print )
            {
               ...
               a /= k*double(pool);
               dlog( "a=${a}   aR=${aR}", ("a", a)("aR", a*gpo.total_vesting_shares.amount.value/BLURT_RC_REGEN_TIME) );
            }
         }
         if( debug_print )
         {
...
void rc_plugin_impl::on_first_block()
{
   ...
   std::string resource_params_json = blurt::jsonball::get_resource_parameters();
   ...
   for( auto it=idx.begin(); it!=idx.end(); ++it )
   {
      create_rc_account( _db, now.sec_since_epoch(), *it, asset( 1000, BLURT_SYMBOL ) );
   }


//   if( op.hardfork_id == BLURT_HARDFORK_0_1 )
//   {
//      const auto& idx = _db.get_index< account_index >().indices().get< by_id >();
//      for( auto it=idx.begin(); it!=idx.end(); ++it )
//      {
...
struct pre_apply_operation_visitor
{
   ...
   void regenerate( const account_object& account, const rc_account_object& rc_account )const
   {
      ...
      // TODO:  Issue number
      //
      static_assert( BLURT_RC_REGEN_TIME <= BLURT_VOTING_MANA_REGENERATION_SECONDS, "RC regen time must be smaller than vote regen time" );
...
struct post_apply_operation_visitor
{
   ...
   void operator()( const clear_null_account_balance_operation& op )const
   {
      _mod_accounts.emplace_back( BLURT_NULL_ACCOUNT );
   }
...
void rc_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   ...
   try
   {
      ...
      if( export_plugin != nullptr )
      {
         ...
         export_plugin->register_export_data_factory( BLURT_RC_PLUGIN_NAME,
            []() -> std::shared_ptr< exportable_block_data > { return std::make_shared< exp_rc_data >(); } );
      }

      chain::database& db = appbase::app().get_plugin< blurt::plugins::chain::chain_plugin >().db();
      ...
      BLURT_ADD_PLUGIN_INDEX(db, rc_resource_param_index);
      BLURT_ADD_PLUGIN_INDEX(db, rc_pool_index);
      BLURT_ADD_PLUGIN_INDEX(db, rc_account_index);
