...
#define BLURT_NAMESPACE_PREFIX "blurt::protocol::"
...
namespace blurt { namespace plugins { namespace account_history_rocksdb {
...
using blurt::protocol::signed_block_header;
using blurt::protocol::signed_transaction;

using blurt::chain::operation_notification;
using blurt::chain::transaction_id_type;

using blurt::utilities::benchmark_dumper;
...
public:
   impl( account_history_rocksdb_plugin& self, const bpo::variables_map& options, const bfs::path& storagePath) :
      _self(self),
      _mainDb(appbase::app().get_plugin<blurt::plugins::chain::chain_plugin>().db()),
      _storagePath(storagePath),
      _writeBuffer(_storage, _columnHandles)
      {
      collectOptions(options);

      _mainDb.add_pre_reindex_handler([&]( const blurt::chain::reindex_notification& note ) -> void
         {
            on_pre_reindex( note );
         }, _self, 0);

      _mainDb.add_post_reindex_handler([&]( const blurt::chain::reindex_notification& note ) -> void
         {
            on_post_reindex( note );
         }, _self, 0);

      BLURT_ADD_PLUGIN_INDEX(_mainDb, volatile_operation_index);
      }
   ...
   void on_pre_reindex( const blurt::chain::reindex_notification& note );
   void on_post_reindex( const blurt::chain::reindex_notification& note );
...
void account_history_rocksdb_plugin::impl::collectOptions(const boost::program_options::variables_map& options)
{
   ...
   BLURT_LOAD_VALUE_SET(options, "account-history-rocksdb-track-account-range", _tracked_accounts, pairstring);
...
std::vector<account_name_type> account_history_rocksdb_plugin::impl::getImpactedAccounts(const operation& op) const
{
   ...
   blurt::app::operation_get_impacted_accounts(op, impacted);
...
void account_history_rocksdb_plugin::impl::storeOpFilteringParameters(const std::vector<std::string>& opList,
   flat_set<std::string>* storage) const
   {
      for(const auto& arg : opList)
      {
         ...
         {
            if( op.empty() == false )
               storage->insert( BLURT_NAMESPACE_PREFIX + op );
         }
      }
   }
...
void account_history_rocksdb_plugin::impl::on_pre_reindex(const blurt::chain::reindex_notification& note)
{
...
void account_history_rocksdb_plugin::impl::on_post_reindex(const blurt::chain::reindex_notification& note)
{
...
FC_REFLECT( blurt::plugins::account_history_rocksdb::account_history_info,
