...
void to_variant( const blurt::plugins::condenser_api::legacy_operation& var,  fc::variant& vo )
{
   var.visit( from_operation( vo ) );
}

void from_variant( const fc::variant& var, blurt::plugins::condenser_api::legacy_operation& vo )
{
   static std::map<string,int64_t> to_tag = []()
   {
      std::map<string,int64_t> name_map;
      for( int i = 0; i < blurt::plugins::condenser_api::legacy_operation::count(); ++i )
      {
         blurt::plugins::condenser_api::legacy_operation tmp;
         tmp.set_which(i);
         string n;
...
void to_variant( const blurt::plugins::condenser_api::legacy_comment_options_extensions& sv, fc::variant& v )
{
   old_sv_to_variant( sv, v );
}

void from_variant( const fc::variant& v, blurt::plugins::condenser_api::legacy_comment_options_extensions& sv )
{
