...
#include <blurt/plugins/account_history_api/account_history_api_plugin.hpp>
...
namespace detail
{
      ...
      public:
            ...
            _db( _chain.db() )
         {
            _on_post_apply_block_conn = _db.add_post_apply_block_handler(
               ...
               appbase::app().get_plugin< blurt::plugins::condenser_api::condenser_api_plugin >(),
               0 );
         }
   ...
   DEFINE_API_IMPL( condenser_api_impl, list_proposals )
   {
      ...
      blurt::plugins::database_api::list_proposals_args list_args;
      list_args.start           = args[0];
      list_args.limit           = args[1].as< uint32_t >();
      list_args.order           = args[2].as< blurt::plugins::database_api::sort_order_type >();
      list_args.order_direction = args.size() > 3 ?
         args[3].as< blurt::plugins::database_api::order_direction_type >() : database_api::ascending;
      list_args.status          = args.size() > 4 ?
         args[4].as< blurt::plugins::database_api::proposal_status >() : database_api::all;
   ...
   DEFINE_API_IMPL( condenser_api_impl, find_proposals )
   {
      ...
      const auto& proposals = _database_api->find_proposals( { args[0].as< vector< blurt::plugins::database_api::api_id_type > >() } ).proposals;
   ...
   void condenser_api_impl::set_pending_payout( discussion& d )
   {
      if( _tags_api )
      {
         ...
         {
            d.promoted = legacy_asset::from_asset( asset( itr->promoted_balance, BLURT_SYMBOL ) );
         }
      }
...
uint16_t api_account_object::_compute_voting_power( const database_api::api_account_object& a )
{
   ...
   // vp_t2 = BLURT_100_PERCENT * current_mana / vests
   // vp_t1 = vp_t2 - BLURT_100_PERCENT * (t2 - t1) / BLURT_VOTING_MANA_REGENERATION_SECONDS
   //

   uint32_t t1 = a.last_vote_time.sec_since_epoch();
   uint32_t t2 = a.voting_manabar.last_update_time;
   uint64_t dt = (t2 > t1) ? (t2 - t1) : 0;
   uint64_t vp_dt = BLURT_100_PERCENT * dt / BLURT_VOTING_MANA_REGENERATION_SECONDS;

   uint128_t vp_t2 = BLURT_100_PERCENT;
   vp_t2 *= a.voting_manabar.current_mana;
   vp_t2 /= vests;

   uint64_t vp_t2u = vp_t2.to_uint64();
   if( vp_t2u >= BLURT_100_PERCENT )
   {
      wlog( "Truncated vp_t2u to BLURT_100_PERCENT for account ${a}", ("a", a.name) );
      vp_t2u = BLURT_100_PERCENT;
   }
...
{
   JSON_RPC_REGISTER_API( BLURT_CONDENSER_API_PLUGIN_NAME );
}
