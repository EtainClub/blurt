...
#include <blurt/plugins/chain/abstract_block_producer.hpp>
#include <blurt/plugins/chain/chain_plugin.hpp>
#include <blurt/plugins/statsd/utility.hpp>

#include <blurt/utilities/benchmark_dumper.hpp>
...
using fc::flat_map;
using blurt::chain::block_id_type;
...
void chain_plugin_impl::write_default_database_config( bfs::path &p )
{
   ...
   fc::json::save_to_file( blurt::utilities::default_database_configuration(), p );
}
...
const blurt::chain::database& chain_plugin::db() const { return my->db; }
...
void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         ("shared-file-size", bpo::value<string>()->default_value("8G"), "Size of the shared memory file. Default: 8G. If running a full node, increase this value to 16G.")
...
#endif
#ifdef IS_TEST_NET
         ("chain-id", bpo::value< std::string >()->default_value( BLURT_CHAIN_ID ), "chain ID to connect to")
#endif
         ;
}
...
void chain_plugin::plugin_startup()
{
   ...
   {
      auto statsd = appbase::app().find_plugin< blurt::plugins::statsd::statsd_plugin >();
      if( statsd != nullptr )
      {
   ...
   blurt::utilities::benchmark_dumper dumper;

   const auto& abstract_index_cntr = my->db.get_abstract_index_cntr();

   typedef blurt::utilities::benchmark_dumper::index_memory_details_cntr_t index_memory_details_cntr_t;
   ...
   db_open_args.initial_supply = BLURT_INIT_SUPPLY;
      ...
      const chainbase::database::abstract_index_cntr_t& abstract_index_cntr )
   {
      ...
      {
         typedef blurt::utilities::benchmark_dumper::database_object_sizeof_cntr_t database_object_sizeof_cntr_t;
      ...
      const blurt::utilities::benchmark_dumper::measurement& measure =
   ...
   if(my->replay)
   {
      ...
      db_open_args.benchmark = blurt::chain::database::TBenchmark(my->benchmark_interval, benchmark_lambda);
      last_block_number = my->db.reindex( db_open_args );

      if( my->benchmark_interval > 0 )
      {
         const blurt::utilities::benchmark_dumper::measurement& total_data = dumper.dump(true, get_indexes_memory_details);
   ...
   {
      db_open_args.benchmark = blurt::chain::database::TBenchmark(dump_memory_details, benchmark_lambda);
...
bool chain_plugin::accept_block( const blurt::chain::signed_block& block, bool currently_syncing, uint32_t skip )
{
...
void chain_plugin::accept_transaction( const blurt::chain::signed_transaction& trx )
{
   boost::promise< void > prom;
   write_context cxt;
   cxt.req_ptr = &trx;
...
blurt::chain::signed_block chain_plugin::generate_block(
   const fc::time_point_sec when,
   const account_name_type& witness_owner,
   const fc::ecc::private_key& block_signing_private_key,
   uint32_t skip )
...
bool chain_plugin::block_is_on_preferred_chain(const blurt::chain::block_id_type& block_id )
{
   ...
   return db().get_block_id_for_num( blurt::chain::block_header::num_from_id( block_id ) ) == block_id;
}

void chain_plugin::check_time_in_block( const blurt::chain::signed_block& block )
{
...
} } } // namespace blurt::plugis::chain::chain_apis
