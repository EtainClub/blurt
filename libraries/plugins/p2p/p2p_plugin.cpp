...
#include <blurt/plugins/p2p/p2p_default_seeds.hpp>
...
using blurt::protocol::block_header;
...
public:
   ...
   virtual blurt::protocol::chain_id_type get_chain_id() const override;
...
blurt::protocol::chain_id_type p2p_plugin_impl::get_chain_id() const
{
...
void p2p_plugin::broadcast_block( const blurt::protocol::signed_block& block )
{
   ulog("Broadcasting block #${n}", ("n", block.block_num()));
   my->node->broadcast( graphene::net::block_message( block ) );
}

void p2p_plugin::broadcast_transaction( const blurt::protocol::signed_transaction& tx )
{
