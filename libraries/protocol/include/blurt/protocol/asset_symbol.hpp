...
#define BLURT_NAI_SHIFT                      ( BLURT_ASSET_SYMBOL_PRECISION_BITS + BLURT_ASSET_CONTROL_BITS )
...
#define BLURT_SYMBOL_U64  (uint64_t('T') | (uint64_t('E') << 8) | (uint64_t('S') << 16) | (uint64_t('T') << 24) | (uint64_t('S') << 32))
...
template< typename Stream >
inline void unpack( Stream& s, blurt::protocol::asset_symbol_type& sym, uint32_t )
{
   uint64_t ser = 0;
   s.read( (char*) &ser, 4 );
...
inline void to_variant( const blurt::protocol::asset_symbol_type& sym, fc::variant& var )
{
   try
   {
...
inline void from_variant( const fc::variant& var, blurt::protocol::asset_symbol_type& sym )
{
   ...
   try
   {
      ...
      FC_ASSERT( decimals->value().as_uint64() <= BLURT_ASSET_MAX_DECIMALS,
         "Expected decimals to be less than or equal to ${num}", ("num", BLURT_ASSET_MAX_DECIMALS) );
