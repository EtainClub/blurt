...
{
   static_assert( BLURT_ASSET_SYMBOL_NAI_STRING_LENGTH >= 12, "This code will overflow a short buffer" );
   uint32_t x = to_nai();
   buf[11] = '\0';
   buf[10] = ((x%10)+'0');  x /= 10;
   buf[ 9] = ((x%10)+'0');  x /= 10;
...
asset_symbol_type asset_symbol_type::from_nai_string( const char* p, uint8_t decimal_places )
{
   try
   {
      ...
      FC_ASSERT( std::strlen( p ) == BLURT_ASSET_SYMBOL_NAI_STRING_LENGTH - 1, "Incorrect NAI string length" );
...
uint32_t asset_symbol_type::asset_num_from_nai( uint32_t nai, uint8_t decimal_places )
{
   ...
   switch( nai_data_digits )
   {
      ...
      default:
         FC_ASSERT( decimal_places <= BLURT_ASSET_MAX_DECIMALS, "Invalid decimal_places" );
         return (nai_data_digits << BLURT_NAI_SHIFT) | SMT_ASSET_NUM_CONTROL_MASK | decimal_places;
   }
}
...
uint32_t asset_symbol_type::to_nai()const
{
   ...
   switch( asset_num )
   {
      ...
      default:
         ...
         nai_data_digits = (asset_num >> BLURT_NAI_SHIFT);
   }
...
void asset_symbol_type::validate()const
{
   switch( asset_num )
   {
      ...
      default:
      {
         uint32_t nai_data_digits = (asset_num >> BLURT_NAI_SHIFT);
...
void asset::validate()const
{
   ...
   FC_ASSERT( amount.value <= BLURT_MAX_SATOSHIS );
}
...
DEFINE_PRICE_COMPARISON_OPERATOR( >= )
      ...
      price price::max( asset_symbol_type base, asset_symbol_type quote ) { return asset( share_type(BLURT_MAX_SATOSHIS), base ) / asset( share_type(1), quote); }
      price price::min( asset_symbol_type base, asset_symbol_type quote ) { return asset( 1, base ) / asset( BLURT_MAX_SATOSHIS, quote); }
...
namespace fc {
   void to_variant( const blurt::protocol::asset& var, fc::variant& vo )
   {
      try
      {
   ...
   void from_variant( const fc::variant& var, blurt::protocol::asset& vo )
   {
      try
      {
         ...
         vo.symbol = blurt::protocol::asset_symbol_type::from_nai_string( v_object[ ASSET_NAI_KEY ].as< std::string >().c_str(), v_object[ ASSET_PRECISION_KEY ].as< uint8_t >() );
      } FC_CAPTURE_AND_RETHROW()
   }
}
