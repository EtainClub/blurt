...
{
   BLURT_STD_ALLOCATOR_CONSTRUCTOR( proposal_object )
   ...
   public:
      ...
      //daily_pay (the amount of daily_pay that is being requested to be paid out daily)
      asset daily_pay;
      ...
      time_point_sec get_end_date_with_delay() const
      {
         ...
         ret += BLURT_PROPOSAL_MAINTENANCE_CLEANUP;
...
{
   BLURT_STD_ALLOCATOR_CONSTRUCTOR( proposal_vote_object )
...
template<> struct is_static_length< blurt::chain::proposal_vote_object > : public boost::true_type {};

} // mira
#endif

FC_REFLECT( blurt::chain::proposal_object, (id)(proposal_id)(creator)(receiver)(start_date)(end_date)(daily_pay)(subject)(permlink)(total_votes)(removed) )
CHAINBASE_SET_INDEX_TYPE( blurt::chain::proposal_object, blurt::chain::proposal_index )

FC_REFLECT( blurt::chain::proposal_vote_object, (id)(voter)(proposal_id) )
CHAINBASE_SET_INDEX_TYPE( blurt::chain::proposal_vote_object, blurt::chain::proposal_vote_index )
