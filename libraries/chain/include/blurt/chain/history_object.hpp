...
namespace blurt { namespace chain {
   ...
   {
      BLURT_STD_ALLOCATOR_CONSTRUCTOR( operation_object )
...
template<> struct is_static_length< blurt::chain::account_history_object > : public boost::true_type {};

} // mira
#endif

FC_REFLECT( blurt::chain::operation_object, (id)(trx_id)(block)(trx_in_block)(op_in_trx)(virtual_op)(timestamp)(serialized_op) )
CHAINBASE_SET_INDEX_TYPE( blurt::chain::operation_object, blurt::chain::operation_index )

FC_REFLECT( blurt::chain::account_history_object, (id)(account)(sequence)(op) )

CHAINBASE_SET_INDEX_TYPE( blurt::chain::account_history_object, blurt::chain::account_history_index )

namespace helpers
{
   template <>
   class index_statistic_provider<blurt::chain::operation_index>
   {
   public:
      typedef blurt::chain::operation_index IndexType;
         ...
         if(onlyStaticInfo == false)
         {
            for(const auto& o : index)
               info._item_additional_allocation +=
                  o.serialized_op.capacity()*sizeof(blurt::chain::buffer_type::value_type);
         }
   ...
   template <>
   class index_statistic_provider<blurt::chain::account_history_index>
   {
   public:
      typedef blurt::chain::account_history_index IndexType;
      ...
      index_statistic_info gather_statistics(const IndexType& index, bool onlyStaticInfo) const
      {
         ...
         if(onlyStaticInfo == false)
         {
            ...
            //      sizeof(blurt::chain::account_history_object::operation_container::value_type);
         }
