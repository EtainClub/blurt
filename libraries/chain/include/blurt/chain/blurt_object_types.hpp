...
#include <blurt/chain/multi_index_types.hpp>

#ifndef ENABLE_MIRA
#define BLURT_STD_ALLOCATOR_CONSTRUCTOR( object_type )   \
      object_type () = delete;                           \
   public:
#else
#define BLURT_STD_ALLOCATOR_CONSTRUCTOR( object_type )   \
...
using blurt::protocol::chain_id_type;
...
template<> struct is_static_length< blurt::protocol::account_name_type > : public boost::true_type {};
template<> struct is_static_length< blurt::protocol::asset_symbol_type > : public boost::true_type {};
template<> struct is_static_length< blurt::protocol::asset > : public boost::true_type {};
template<> struct is_static_length< blurt::protocol::price > : public boost::true_type {};
...
#ifndef ENABLE_MIRA
inline void to_variant( const blurt::chain::shared_string& s, variant& var )
{
   var = fc::string( blurt::chain::to_string( s ) );
}

inline void from_variant( const variant& var, blurt::chain::shared_string& s )
{
...
{
   std::string str = blurt::chain::to_string( ss );
   fc::raw::pack( s, str );
}
...
void unpack( Stream& s, chainbase::shared_string& ss, uint32_t depth )
{
   ...
   blurt::chain::from_string( ss, str );
}
#endif
...
#ifndef ENABLE_MIRA
template< typename T >
T unpack_from_vector( const blurt::chain::buffer_type& s )
{
   try
   {
      T tmp;
...
FC_REFLECT_ENUM( blurt::chain::object_type,
                 (dynamic_global_property_object_type)
                 (account_object_type)
                 (account_metadata_object_type)
                 (account_authority_object_type)
...
#ifndef ENABLE_MIRA
FC_REFLECT_TYPENAME( blurt::chain::shared_string )
#endif

FC_REFLECT_ENUM( blurt::chain::bandwidth_type, (post)(forum)(market) )
