...
// time.
//
#ifndef BLURT_BLOCKCHAIN_STATISTICS_SPACE_ID
#define BLURT_BLOCKCHAIN_STATISTICS_SPACE_ID 9
#endif

#ifndef BLURT_BLOCKCHAIN_STATISTICS_PLUGIN_NAME
#define BLURT_BLOCKCHAIN_STATISTICS_PLUGIN_NAME "chain_stats"
#endif
...
{
   bucket_object_type = ( BLURT_BLOCKCHAIN_STATISTICS_SPACE_ID << 8 )
};
...
class blockchain_statistics_plugin : public blurt::app::plugin
{
   public:
      blockchain_statistics_plugin( application* app );
      virtual ~blockchain_statistics_plugin();

      virtual std::string plugin_name()const override { return BLURT_BLOCKCHAIN_STATISTICS_PLUGIN_NAME; }
...
FC_REFLECT( blurt::blockchain_statistics::bucket_object,
   (id)
   (open)
   (seconds)
   (blocks)
