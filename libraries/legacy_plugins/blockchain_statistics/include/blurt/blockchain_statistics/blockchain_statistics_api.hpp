...
#include <blurt/blockchain_statistics/blockchain_statistics_plugin.hpp>
...
{
   public:
      blockchain_statistics_api( const blurt::app::api_context& ctx );
...
FC_REFLECT( blurt::blockchain_statistics::statistics,
   (blocks)
   (bandwidth)
   (operations)
   (transactions)
