...
namespace detail
{
   ...
   {
      public:
         blockchain_statistics_api_impl( blurt::app::application& app )
            :_app( app ) {}
   ...
   statistics blockchain_statistics_api_impl::get_stats_for_interval( fc::time_point_sec start, fc::time_point_sec end )const
   {
      ...
      const auto& sizes = _app.get_plugin< blockchain_statistics_plugin >( BLURT_BLOCKCHAIN_STATISTICS_PLUGIN_NAME )->get_tracked_buckets();
...
blockchain_statistics_api::blockchain_statistics_api( const blurt::app::api_context& ctx )
{
...
statistics& statistics::operator +=( const bucket_object& b )
{
   ...
   this->blurt_transferred                      += b.blurt_transferred;
   ...
   this->blurt_vested                           += b.blurt_vested;
