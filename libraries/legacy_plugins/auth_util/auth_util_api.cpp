...
{
   public:
      auth_util_api_impl( blurt::app::application& _app );
      void check_authority_signature( const check_authority_signature_params& args, check_authority_signature_result& result );

      std::shared_ptr< blurt::plugin::auth_util::auth_util_plugin > get_plugin();

      blurt::app::application& app;
};

auth_util_api_impl::auth_util_api_impl( blurt::app::application& _app ) : app( _app )
{}

std::shared_ptr< blurt::plugin::auth_util::auth_util_plugin > auth_util_api_impl::get_plugin()
{
...
auth_util_api::auth_util_api( const blurt::app::api_context& ctx )
{
