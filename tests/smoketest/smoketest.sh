...
function print_help_and_quit {
   echo "Usage: path_to_tested_blurtd path_to_reference_blurtd path_to_test_blockchain_directory"
   echo "       path_to_reference_blockchain_directory number_of_blocks_to_replay [number_of_jobs [--dont-copy-config]]"
   echo "Example: ~/work/steemit/steem/build/Release/programs/blurtd/blurtd ~/master/steemit/steem/build/Release/programs/blurtd/blurtd ~/steemit/steem/work1 ~/steemit/steem/work2 5000000"
...
function run_test_group {
   ...
   echo Running ./$GROUP_TEST_SCRIPT $JOBS $TEST_BLURTD_PATH $REF_BLURTD_PATH $TEST_WORK_PATH $REF_WORK_PATH $BLOCK_LIMIT $COPY_CONFIG
   ./$GROUP_TEST_SCRIPT $JOBS $TEST_BLURTD_PATH $REF_BLURTD_PATH $TEST_WORK_PATH $REF_WORK_PATH $BLOCK_LIMIT $COPY_CONFIG
   EXIT_CODE=$?
   if [ $EXIT_CODE -ne 0 ]
   then
      EXIT_CODE=-1
...
check_blurtd_path $TEST_BLURTD_PATH
check_blurtd_path $REF_BLURTD_PATH
