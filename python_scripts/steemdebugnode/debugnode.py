...
class DebugNode( object ):
   """ Wraps the blurtd debug node plugin for easier automated testing of the Steem Network"""

   def __init__( self, blurtd, data_dir, args='', plugins=[], apis=[], blurtd_out=None, blurtd_err=None ):
      """ Creates a blurtd debug node.
      ...
      args:
         blurtd -- The string path to the location of the blurtd binary
         data_dir -- The string path to an existing blurtd data directory which will be used to pull blocks from.
         args -- Other string args to pass to blurtd.
         ...
         blurtd_stdout -- A stream for blurtd's stdout. Default is to pipe to /dev/null
         blurtd_stderr -- A stream for blurtd's stderr. Default is to pipe to /dev/null
      """
      self._data_dir = None
      self._debug_key = None
      self._FNULL = None
      self._rpc = None
      self._blurtd_bin = None
      self._blurtd_lock = None
      self._blurtd_process = None
      self._temp_data_dir = None

      self._blurtd_bin = Path( blurtd )
      if( not self._blurtd_bin.exists() ):
         raise ValueError( 'blurtd does not exist' )
      if( not self._blurtd_bin.is_file() ):
         raise ValueError( 'blurtd is not a file' )
      ...
      if( blurtd_out != None ):
         self.blurtd_out = blurtd_out
      else:
         self.blurtd_out = self._FNULL

      if( blurtd_err != None ):
         self.blurtd_err = blurtd_err
      else:
         self.blurtd_err = self._FNULL

      self._debug_key = '5JHNbFNDg834SFj8CMArV6YW7td4zrPzXveqTfaShmYVuYNeK69'
      self._blurtd_lock = Lock()


   def __enter__( self ):
      self._blurtd_lock.acquire()
      ...
      blurtd = [ str( self._blurtd_bin ), '--data-dir=' + str( self._temp_data_dir.name ) ]
      blurtd.extend( self._args )

      self._blurtd_process = Popen( blurtd, stdout=self.blurtd_out, stderr=self.blurtd_err )
      self._blurtd_process.poll()
      sleep( 5 )
      if( not self._blurtd_process.returncode ):
         self._rpc = SteemNodeRPC( 'ws://127.0.0.1:8095', '', '' )
      else:
         raise Exception( "blurtd did not start properly..." )

   def __exit__( self, exc, value, tb ):
      self._rpc = None

      if( self._blurtd_process != None ):
         self._blurtd_process.poll()

         if( not self._blurtd_process.returncode ):
            self._blurtd_process.send_signal( SIGINT )

            sleep( 7 )
            self._blurtd_process.poll()

            if( not self._blurtd_process.returncode ):
               self._blurtd_process.send_signal( SIGTERM )

               sleep( 5 )
               self._blurtd_process.poll()

               if( self._blurtd_process.returncode ):
                  loggin.error( 'blurtd did not properly shut down after SIGINT and SIGTERM. User intervention may be required.' )

      self._blurtd_process = None
      self._temp_data_dir.cleanup()
      self._temp_data_dir = None
      self._blurtd_lock.release()
   ...
   def debug_set_hardfork( self, hardfork_id ):
      ...
      args:
         hardfork_id: The id of the hardfork to set. Hardfork IDs start at 1 (0 is genesis) and increment
            by one for each hardfork. The maximum value is BLURT_NUM_HARDFORKS in chain/hardfork.d/0-preamble.hf
      """
      if( hardfork_id < 0 ):
...
if __name__=="__main__":
   ...
   def main():
      global WAITING
      """
      This example contains a simple parser to obtain the locations of both blurtd and the data directory,
      ...
      parser = ArgumentParser( description='Run a Debug Node on an existing chain. This simply replays all blocks ' + \
                                 ...
                                 'the CLI wallet' )
      parser.add_argument( '--blurtd', '-s', type=str, required=True, help='The location of a blurtd binary to run the debug node' )
      ...
      blurtd = Path( args.blurtd )
      if( not blurtd.exists() ):
         print( 'Error: blurtd does not exist.' )
         return

      blurtd = blurtd.resolve()
      if( not blurtd.is_file() ):
         print( 'Error: blurtd is not a file.' )
         return

      data_dir = Path( args.data_dir )
      if( not data_dir.exists() ):
         print( 'Error: data_dir does not exist or is not a properly constructed blurtd data directory' )
      ...
      debug_node = DebugNode( str( blurtd ), str( data_dir ), blurtd_err=sys.stderr )
