...
VERSION=`cat /etc/blurtdversion`

if [[ "$IS_BROADCAST_NODE" ]]; then
  BLURTD="/usr/local/blurtd-default/bin/blurtd"
elif [[ "$IS_AH_NODE" ]]; then
  BLURTD="/usr/local/blurtd-default/bin/blurtd"
else
  BLURTD="/usr/local/blurtd-full/bin/blurtd"
fi

chown -R blurtd:blurtd $HOME
...
if [[ ! -z "$BLURTD_SEED_NODES" ]]; then
    for NODE in $BLURTD_SEED_NODES ; do
        ARGS+=" --p2p-seed-node=$NODE"
    done
fi

NOW=`date +%s`
BLURTD_FEED_START_TIME=`expr $NOW - 1209600`

ARGS+=" --follow-start-feeds=$BLURTD_FEED_START_TIME"

BLURTD_PROMOTED_START_TIME=`expr $NOW - 604800`
ARGS+=" --tags-start-promoted=$BLURTD_PROMOTED_START_TIME"
...
if [[ "$IS_BROADCAST_NODE" ]]; then
  cp /etc/blurtd/config-for-broadcaster.ini $HOME/config.ini
elif [[ "$IS_AH_NODE" ]]; then
  cp /etc/blurtd/config-for-ahnode.ini $HOME/config.ini
elif [[ "$IS_OPSWHITELIST_NODE" ]]; then
  cp /etc/blurtd/fullnode.opswhitelist.config.ini $HOME/config.ini
else
  cp /etc/blurtd/fullnode.config.ini $HOME/config.ini
fi

chown blurtd:blurtd $HOME/config.ini

cd $HOME

mv /etc/nginx/nginx.conf /etc/nginx/nginx.original.conf
cp /etc/nginx/blurtd.nginx.conf /etc/nginx/nginx.conf

# get blockchain state from an S3 bucket
echo blurtd: beginning download and decompress of s3://$S3_BUCKET/blockchain-$VERSION-latest.tar.lz4
finished=0
count=1
if [[ "$USE_RAMDISK" ]]; then
  mkdir -p /mnt/ramdisk
  ...
  do
    ...
    if [[ $? -ne 0 ]]; then
      sleep 1
      echo notifyalert blurtd: unable to pull blockchain state from S3 - attempt $count
      (( count++ ))
    else
      finished=1
    fi
  done
  chown -R blurtd:blurtd /mnt/ramdisk/blockchain
else
  while [[ $count -le 5 ]] && [[ $finished == 0 ]]
  do
    rm -rf $HOME/blockchain/*
...
if [[ $finished == 0 ]]; then
  if [[ ! "$SYNC_TO_S3" ]]; then
    echo notifyalert blurtd: unable to pull blockchain state from S3 - exiting
    exit 1
  else
    echo notifyblurtdsync blurtdsync: shared memory file for $VERSION not found, creating a new one by replaying the blockchain
    if [[ "$USE_RAMDISK" ]]; then
    ...
    if [[ $? -ne 0 ]]; then
      echo notifyblurtdsync blurtdsync: unable to pull latest block_log from S3, will sync from scratch.
    else
...
echo $SAVED_PID >> /tmp/blurtdpid
mkdir -p /etc/service/blurtd
if [[ ! "$SYNC_TO_S3" ]]; then
  cp /usr/local/bin/paas-sv-run.sh /etc/service/blurtd/run
else
  cp /usr/local/bin/sync-sv-run.sh /etc/service/blurtd/run
fi
